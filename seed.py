...
through the same ORM models the app itself uses is. Calls init_db()
itself, so an empty database is fine — no need for the app to have
started first. Rerunning it doesn't duplicate users/research programs,
but does duplicate time_requests.
